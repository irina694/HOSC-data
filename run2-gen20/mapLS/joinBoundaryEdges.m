function map = joinBoundaryEdges(map)

    C = map.connectivity;
    N=C.Ncell;   
    vertType = map.cells.vertices.type;
    vertLR = map.cells.vertices.LR;
    boundaryLR = map.cells.boundary.boundariesLR;
    boundLRFix = map.cells.boundary.boundLRFixed;
    NBtypes = length(boundaryLR);
    for i=1:N        
        vert2del=[];
        %newcont=[];
        cellvert = C.cell{i};
        ...
        candidatesLR = vertLR(cellvert,:);        
        candidatesLR = candidatesLR(:,1)*10+candidatesLR(:,2);
                
        for k=1:NBtypes
            candidates = find(candidatesLR==boundaryLR(k)|candidatesLR==boundLRFix(k)|candidatesLR==boundLRFix(k+1));
            [aux1,fstV,lstV]=findVert2del(candidates,Nvert);
            vert2del=[vert2del,cellvert(aux1)]; 
            C = updateCedges(C,cellvert([fstV,aux1,lstV]));
        end        
        ...
        %labels the derts to delete as unused in the cells.vetices structure
        map.cells.vertices.type(vert2del)='U';
    end
