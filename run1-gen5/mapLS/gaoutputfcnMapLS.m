function [state, options,optchanged] = gaoutputfcnMapLS(options,state,flag,p1,p2)
global Gvars savestate individuals mapPopulation
...
switch flag
 case 'init'
        if(~isempty(savestate)) %it restarting a problem
            ...
            aux = state;
            aux.mapPopulation = mapPopulation;
