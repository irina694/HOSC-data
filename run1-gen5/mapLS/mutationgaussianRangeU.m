function mutationChildren = mutationgaussianRangeU(parents,options,GenomeLength,FitnessFcn,state,thisScore,thisPopulation)
global nVert flagVert
...
if(strcmpi(options.PopulationType,'doubleVector'))
    ...
    upper = range(2,:);
    mean  = (lower+upper)/2;
    scale = scale * (upper-lower)/2;
    
    mutationChildren = zeros(length(parents),GenomeLength);
    for i=1:length(parents)
        parent = thisPopulation(parents(i),:);
            for j=1:GenomeLength
%                 flagV=0;    %genes must be within the initial range
%                 while flagV==0
                     candidate=parent(j)+scale(j)*rand+mean(j);
%                     if (candidate>=lower(j))&&(candidate<=upper(j))
                        mutationChildren(i,j)=candidate;
%                         flagV=1;
%                     end
%                 end
            end            
